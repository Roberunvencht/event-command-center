...
import { Trophy, Download, Activity, Heart, AlertTriangle, TrendingUp, FileText } from "lucide-react";
...
export default function Reports() {
  ...
  const leaderboardData = {
    ...
    "full-marathon": [
      { rank: 1, bibNumber: "001", name: "John Doe", category: "Full Marathon", time: "2:10:45", avgHR: 165, alerts: 0 },
      { rank: 2, bibNumber: "002", name: "Jane Smith", category: "Full Marathon", time: "2:12:20", avgHR: 162, alerts: 1 },
      { rank: 3, bibNumber: "003", name: "Sarah Johnson", category: "Full Marathon", time: "2:18:45", avgHR: 158, alerts: 2 },
    ],
    "half-marathon": [
      { rank: 1, bibNumber: "042", name: "Mike Wilson", category: "Half Marathon", time: "1:05:30", avgHR: 170, alerts: 0 },
      { rank: 2, bibNumber: "043", name: "Emily Davis", category: "Half Marathon", time: "1:08:15", avgHR: 168, alerts: 1 },
    ],
    "10k": [
      { rank: 1, bibNumber: "005", name: "Tom Brown", category: "10K", time: "35:20", avgHR: 175, alerts: 0 },
      { rank: 2, bibNumber: "006", name: "Lisa Anderson", category: "10K", time: "37:45", avgHR: 172, alerts: 0 },
    ],
  };
  ...
  const medicalProfiles = [
    ...
    },
    {
      ...
      alerts: [
        { time: "01:45:30", type: "Elevated HR", severity: "Warning", message: "Heart rate above 180 bpm" }
      ]
    },
    {
      bibNumber: "003",
  ...
  const exportLeaderboardPDF = async () => {
    ...
    pdf.setFontSize(12);
    pdf.text(`Event: ${events.find(e => e.id === selectedEvent)?.name}`, 15, 25);
    pdf.text(`Category: ${selectedCategory === 'all' ? 'All Categories' : selectedCategory.replace('-', ' ')}`, 15, 32);
  ...
  const exportMedicalReportPDF = async (runner: typeof medicalProfiles[0]) => {
    ...
    pdf.text(`Zone 1: ${runner.heartRate.zones.z1}% | Zone 2: ${runner.heartRate.zones.z2}% | Zone 3: ${runner.heartRate.zones.z3}%`, 20, 91);
    pdf.text(`Zone 4: ${runner.heartRate.zones.z4}% | Zone 5: ${runner.heartRate.zones.z5}%`, 20, 97);
    ...
    if (runner.alerts.length > 0) {
      ...
      runner.alerts.forEach((alert, index) => {
        pdf.text(`${index + 1}. [${alert.time}] ${alert.type} (${alert.severity})`, 20, yPos);
    ...
    pdf.save(`medical-report-${runner.bibNumber}-${runner.name.replace(/\s+/g, '-')}.pdf`);
  };

  const currentLeaderboard = leaderboardData[selectedCategory as keyof typeof leaderboardData] || leaderboardData.all;
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Event performance reports and medical analytics</p>
        </div>
      </div>
          ...
          <Card id="leaderboard-section">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button onClick={exportLeaderboardPDF} variant="outline" size="sm">
            ...
            <CardContent className="space-y-4">
              <div className="flex gap-2">
                ...
                <Button 
                  variant={selectedCategory === "full-marathon" ? "default" : "outline"} 
                  size="sm"
                ...
                <Button 
                  variant={selectedCategory === "half-marathon" ? "default" : "outline"} 
                  size="sm"
                ...
                <Card key={runner.bibNumber} className="border-2">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      <Button onClick={() => exportMedicalReportPDF(runner)} variant="outline" size="sm">
                  ...
                  <CardContent className="space-y-4">
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-semibold">{runner.heartRate.avg} bpm</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span className="text-muted-foreground">Min / Max:</span>
                            <span className="font-semibold">{runner.heartRate.min} / {runner.heartRate.max} bpm</span>
                          </div>
                            ...
                            <div className="flex gap-1">
                              {Object.entries(runner.heartRate.zones).map(([zone, percent]) => (
                                <div key={zone} className="flex-1">
                                  ...
                                  <p className="text-xs text-center mt-1">{percent}%</p>
                                </div>
                              ))}
                            </div>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <Badge variant={
                              runner.semg.fatigueLevel === "Low" ? "outline" : 
                              runner.semg.fatigueLevel === "Moderate" ? "secondary" : 
                              "destructive"
                            }>
                          ...
                          <div className="mt-3">
                            <div className="w-full h-2 bg-muted rounded overflow-hidden">
                              <div 
                                className={`h-full ${
                                  runner.semg.fatigueLevel === "Low" ? "bg-green-500" :
                                  runner.semg.fatigueLevel === "Moderate" ? "bg-yellow-500" :
                                  "bg-red-500"
                                }`}
                    ...
                    {/* Alerts */}
                    <Card className={runner.alerts.length > 0 ? "border-destructive" : "border-green-500"}>
                      <CardHeader className="pb-3">
                        <CardTitle className="text-sm flex items-center gap-2">
                          <AlertTriangle className={`w-4 h-4 ${runner.alerts.length > 0 ? "text-destructive" : "text-green-500"}`} />
                        ...
                        {runner.alerts.length > 0 ? (
                          <div className="space-y-3">
                            {runner.alerts.map((alert, index) => (
                              <div key={index} className="flex items-start gap-3 p-3 border border-border rounded-lg bg-destructive/5">
                          ...
                          <p className="text-sm text-muted-foreground flex items-center gap-2">
                            ...
                            No alerts triggered. Runner maintained healthy vitals throughout the race.
                          </p>
                        )}
                      </CardContent>
                    </Card>
