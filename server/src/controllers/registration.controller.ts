...
export const registerController =  asyncHandler(async (req, res) => {
    ...
    const category = event.raceCategories.find(cat => cat._id.toString() === raceCategoryId);
    appAssert(category, NOT_FOUND, 'Race category not found');

    appAssert(category.registeredCount < category.slots, BAD_REQUEST, 'Category full');
    ...
    res.status(CREATED).json(new CustomResponse(true, registration, 'Registration successful'));
})